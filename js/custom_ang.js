  ...
  $scope.updatePass = function () {
    ...
    if (jQuery("#lib_password_form").valid()) {
      ...
      jQuery.ajax({
        ...
        success: function (res) {
          showMessage("Password updated successfully", "OK", "success");
...
app.controller('UserApprovalCtrl', function ($scope, $compile, $http) {
  ...
  $scope.btn_approve = function (event) {
    ...
    bootbox.confirm("Are your sure to approve this user?", function (result) {
      if (result) {
        jQuery.ajax({
          type: 'POST',
          url: myAjax.ajaxurl,
          ...
          success: function (res) {
            if (res.success) {
              showMessage("User has been approved!.", "Success", "success");
              load_user_data_ajax(limit_pg_no, 0, "", "", "", "", $compile, $scope);
            }
                ...
                if (result) {
                  jQuery.ajax({
                    ...
                    success: function (res) {
                      res = JSON.parse(res);
                      if (res.success) {
                        showMessage("User has been deleted!.", "Success", "success");
                        load_user_data_ajax(limit_pg_no, 0, "", "", "", "", $compile, $scope);
                      }
        ...
        success: function (res) {
          ...
          if (res.success == false) {
            ...
            if (res.msg.indexOf("Isbn") >= 0) {
              bootbox.confirm("Add book to this ISBN?", function (result) {
                if (result) {
                  jQuery.ajax({
                    type: 'POST',
                    url: myAjax.ajaxurl,
    ...
    if ($scope.book_isbn !== undefined) {
      if ($scope.book_isbn.length >= 6) {
        jQuery.ajax({
          ...
          dataType: 'JSON',
          beforeSend: function () {
            showMessage("We are serching for the book details. Plz Wait..", "Status", "info");
          },
          success: function (res) {
            if (typeof res.items != "undefined") {
              ...
              hide_msg();
              showMessage("We couldn't find the book you are looking for so switched to manual mode ..", "Status", "info");
...
app.controller('saveYearsCtrl', function ($scope, $compile, $http, $window) {
  ...
  $scope.updatebtn = function () {
    if ($scope.edit_year != "") {
      ...
      showMessage("Content data is missing.", "Information", "info");
    }
  }
              ...
              if (res.data != "") {
                ...
                jQuery.ajax({
                  ...
                  success: function (res) {
                    res = JSON.parse(res);
                    if (res.success) {
                      showMessage("User has been deactivated!.", "Success", "success");
                      location.reload();
                      //load_user_data_ajax(limit_pg_no, 0, "", "", "", "", $compile, $scope);
                    }
...
app.controller('managementofbooksCtrl', function ($scope, $compile, $http, $window, $parse, Upload) {
  ...
  $scope.btn_delBookDetails = function ($book_id) {
    bootbox.confirm("Are your sure about to delete this book?", function (result) {
      if (result) {
        jQuery.ajax({
          type: 'POST',
          url: myAjax.ajaxurl,
  ...
  $scope.onBookchanged = function ($x) {
    ...
    if ($available != 0) {
      ...
      showMessage("Book has been issued so get it returned before disabling it.", "Info", "warning");
    }
        ...
        success: function (res) {
          ...
          if (res.data.length == 1) {
            ...
            if (res.msg != null) {
              ...
              showMessage("Book has already been Issued.", "Info", "info");
              $scope.btn_issue_status = true;
            }
            $scope.qty = "N/A";
            $scope.price = "N/A";
...
app.controller('managementofissuedbooksUserCtrl', function ($scope, $compile, $http, $window, $parse, Upload) {
  ...
  $scope.openPaymentPage = function (x) {
    ...
    jQuery("#merchant_param2").val(x.UserId);
    jQuery("#merchant_param3").val(x.BookId);
...
app.controller('managementofissuedbooksCtrl', function ($scope, $compile, $http, $window, $parse, Upload) {
  ...
  $scope.viewSmsModal = function (x) {
    ...
    $scope.email_body = "Book Issued {" + x.BookName + "} to you needs to be return to the library. ";
    $scope.sms_body = "Book Issued {" + x.BookName + "} to you needs to be return to the library. ";
...
app.controller('viewUserRequestBookCtrl', function ($scope, $compile, $http, $window, $parse, Upload, $rootScope) {
  ...
  $scope.sendRequest = function () {
    if (jQuery("#lib_request_book").valid()) {
      bootbox.confirm("Are your sure to submit the request?", function (result) {
        if (result) {
          jQuery.ajax({
            type: 'POST',
            url: myAjax.ajaxurl,
...
app.controller('viewrequestBookCtrl', function ($scope, $compile, $http, $window, $parse, Upload, $rootScope) {
  ...
  $scope.approve = function (x) {
    bootbox.confirm("Are your sure to approve the request?", function (result) {
      if (result) {
        jQuery.ajax({
          type: 'POST',
          url: myAjax.ajaxurl,
  ...
  $scope.dissapprove = function (x) {
    bootbox.confirm("Are your sure to dissapprove the request?", function (result) {
      if (result) {
        jQuery.ajax({
          type: 'POST',
          url: myAjax.ajaxurl,
  ...
  $scope.delete = function (x) {
    bootbox.confirm("Are your sure to delete this request?", function (result) {
      if (result) {
        jQuery.ajax({
          type: 'POST',
          url: myAjax.ajaxurl,
...
app.controller('saveSlidesCtrl', function ($scope, $compile, $http, $window, $parse, Upload, $rootScope) {
  ...
  $scope.editSlide = function (slide) {
    ...
    showMessage("Edit mode set.Now upload the image.", "OK", "info");
  }
  $scope.addSlide = function () {
    ...
    if (files != null) {
      ...
      showMessage("No file selected.Kindly select an Image file to upload", "OK", "info");
    }
...
app.controller('duepaidCtrl', function ($scope) {
  ...
  $scope.updateStatus = function (x) {
    bootbox.confirm("Are your sure to approve this due? <br/><b>Note :This action is isreversible.</b>", function (result) {
      if (result) {
        jQuery.ajax({
          type: 'POST',
          url: myAjax.ajaxurl,
...
app.controller('CtrlUpdateManager', function ($scope) {
  ...
  jQuery("#shell-body").append("<li>Now you can update by just a single click.</li>");
  jQuery("#shell-body").append("<li>Note: All the files which are updated is backuped in 'bck' folder.");
  jQuery("#shell-body").append("<li>*****************************************************************</li>");
  jQuery("#shell-body").append("<li>---------START--------</li>");
    ...
    jQuery.ajax({
      ...
      success: function (res) {
        if (res !== undefined && res["msg"] == "") {
          ...
          setTimeout(function () {
            jQuery("#shell-body").append("<li>Reading update files...</li><li>" + res["count"] + " files found to be updated.</li> <li>Preparing file for update...</li><li>Backing up code in bck/ folder...</li>");
          }, 3000);
          setTimeout(function () {
          ...
          setTimeout(function () {
            jQuery("#shell-body").append("<li>Updated " + res["count"] + " files.</li><li>All files are updated.</li><li>---------END--------</li>");
          }, 5000);
        }
        else {
          setTimeout(function () {
