...
//add_action('wp_ajax_save_img', 'FuncSaveImage');
//add_action('wp_ajax_nopriv_save_img', 'FuncSaveImage');
//function FuncSaveImage()
//{
...
function upload_pdf($file_name, $unique_id)
{
    ...
    $ext = pathinfo($filename, PATHINFO_EXTENSION);
    if (!in_array($ext, $allowed)) {
        echo json_encode(array('success' => true, "msg" => "Only pdf should be uploaded!", 'color' => 'info', 'header' => "Info"));
        wp_die();
    }
    $attachment_id = media_handle_upload($file_name, $unique_id);
...
{
    $attach_id = sanitize_text_field($_REQUEST["img_id"]);
