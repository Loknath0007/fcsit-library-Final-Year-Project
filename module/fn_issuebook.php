...
function issue_book_to_user()
{
    ...
    $sub_book_id = sanitize_text_field($_REQUEST["book_no"]);
    $user_id = sanitize_text_field($_REQUEST["user_id"]);
    $notes = sanitize_text_field($_REQUEST["notes"]);
    $book_date_borrowed = sanitize_text_field($_REQUEST["book_date_borrowed"]);
    $book_date_due = sanitize_text_field($_REQUEST["book_date_due"]);
...
//    };
    if ($temp_data < $limit) {
        ...
        echo json_encode(array('success' => true, 'msg' => "User Limit Has reached!", "header" => "OK", 'color' => 'info'));
        wp_die();
    }
}
...
function get_all_issued_book()
{
    ...
    {
        $query_ext.="and UserId=".sanitize_text_field($_REQUEST["user_id"]);
    }
...
function return_issued_book()
{
    ...
    $user_id = sanitize_text_field($_REQUEST["user_id"]);
    $notes = sanitize_text_field($_REQUEST["notes"]);
    $fine = sanitize_text_field($_REQUEST["fine"]);
    $book_return_date = sanitize_text_field($_REQUEST["book_return_date"]);
    $delay_day = sanitize_text_field($_REQUEST["delay_day"]);
    ...
    echo json_encode(array('success' => true, 'msg' => "Book has been returned!", "header" => "OK", 'color' => 'success'));
    wp_die();
}

function return_issued_book_hardcode($bookno, $userid, $book_return_date, $fine_amount, $delay_day, $uid)
{
    global $wpdb;
    $sub_book_id = sanitize_text_field($bookno);
    $user_id = sanitize_text_field($userid);
    $notes = 'Online Mode Payment';
    $fine = sanitize_text_field($fine_amount);
    $book_return_date = sanitize_text_field($book_return_date);
    $delay_day = sanitize_text_field($delay_day);
    ...
    $sql_fire1 = "update tblborrowed set DateReturned='" . $book_return_date . "',Notes='" . $notes . "',DelayedDay=" . $delay_day . ",ReturnStatus=1,Fine=" . $fine . " where BookId=" . $sub_book_id . " and UserId=" . $user_id . " and Id=" . $uid;
    $sql_fire2 = "update tblsubbooks set Available = 1 where BookId=" . $sub_book_id . "";
    $sql_fire3 = "update tblbooks set Borrowed=Borrowed-1 where Id = (select ParentBookID from tblsubbooks where BookId=" . $sub_book_id . ")";
    $wpdb->query($sql_fire1);
    $wpdb->query($sql_fire2);
