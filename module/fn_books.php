...
{
    $book_isbn = trim(sanitize_text_field($_REQUEST["book_isbn"]));
    $book_author = sanitize_text_field($_REQUEST["book_author"]);
    $book_title = sanitize_text_field($_REQUEST["book_title"]);
    $book_category = sanitize_text_field($_REQUEST["book_category"]);
    $book_publisher = sanitize_text_field($_REQUEST["book_publisher"]);
    $book_desc = sanitize_text_field($_REQUEST["book_desc"]);
    $book_url = sanitize_text_field($_REQUEST["book_url"]);
    $book_goo_id = sanitize_text_field($_REQUEST["book_goo_id"]);
    $book_src = sanitize_text_field($_REQUEST["book_src"]);
    $book_external_url = sanitize_text_field($_REQUEST["book_external_url"]);
//   if ($book_src == "") {
//        $book_src = sanitize_text_field($_REQUEST["book_tmp_image_link"]);
//    }
    $pic_id = "";
    ...
    $book_qty = sanitize_text_field($_REQUEST["book_qty"]);
    ...
    if ($temp_check->count == 0) {
        ...
        if ($id_updated != 0) {
            ...
            echo json_encode(array('success' => true, "msg" => "Adding of books is done!", 'color' => 'success', 'header' => "Success"));
            wp_die();
        } else {
            echo json_encode(array('success' => true, "msg" => "Adding of book failed!", 'color' => 'info', 'header' => "Info"));
            wp_die();
        }
    } else {
        echo json_encode(array('success' => false, "msg" => "Isbn already exist!", 'color' => 'info', 'header' => "Info"));
        wp_die();
    }
}
...
function FuncAddSubBooks()
{
    ...
    echo json_encode(array('success' => true, "msg" => "Adding of books is done!", 'color' => 'success', "header" => "Info"));
    wp_die();
}
...
function delete_approval_check_book_issued()
{
    ...
    if (isset($_REQUEST["user_id"]) && $_REQUEST["user_id"] != "") {
        ...
        echo json_encode(array('success' => true, 'msg' => "User Id coudn't be found.", 'color' => 'info', 'header' => 'Success'));
    }
    wp_die();
}
...
{
    global $wpdb;
    $book_id = sanitize_text_field($_REQUEST["selected_book_id"]);
    $book_publisher = sanitize_text_field($_REQUEST["selected_book_publisher"]);
    $book_title = sanitize_text_field($_REQUEST["selected_book_title"]);
    $book_category = sanitize_text_field($_REQUEST["select_book_category"]);
    $book_author = sanitize_text_field($_REQUEST["select_book_author"]);
    $book_price = sanitize_text_field($_REQUEST["select_book_price"]);
    $book_desc = sanitize_text_field($_REQUEST["selected_book_desc"]);
    $book_img = sanitize_text_field($_REQUEST["select_book_img"]);
    $book_external_url = sanitize_text_field($_REQUEST["select_book_external_url"]);
    $select_book_preview_lnk = sanitize_text_field($_REQUEST["select_book_preview_lnk"]);
    ...
    if ($book_img == null || $book_img == "") {
        echo json_encode(array('success' => true, "msg" => "Kindly Specify a image!", "header" => "Issue", 'color' => 'error'));
        exit();
    }
...
//    }
    ...
    //exit();
    if ($id_updated >= 0) {
        echo json_encode(array('success' => true, "msg" => "Updating of book has been done!", "header" => "OK", 'color' => 'success'));
    } else {
        echo json_encode(array('success' => true, "msg" => "Some error occured!", "header" => "Issue", 'color' => 'info'));
    }
    wp_die();
}
    ...
    if (!empty($full_data)) {
        foreach ($full_data as $book_obj) {
            ...
            if ($book_obj->BookDesc != "") {
                $dynamic_html .= '<td style="text-align:left;" class="wrap_book_desc book_desc_mng">' . wp_trim_words($book_obj->BookDesc, 100, '...') . '</td>';
            } else {
...
function update_specific_book_detail()
{
    ...
    echo json_encode(array('success' => true, "msg" => "Book Data has been updated !", "header" => "OK", 'color' => 'success'));
    wp_die();
}
...
function delete_book_info()
{
    ...
    $dynamic_html = "";
    $query_ext = "";
    $parent_id = sanitize_text_field($_REQUEST["bid"]);
    ...
    echo json_encode(array('success' => true, "header" => "OK", 'msg' => 'Book has been deleted successfully.', 'color' => 'success'));
    wp_die();
}
...
function get_specific_book_details()
{
    ...
    if ($temp_data != "0") {
        $msg = "Book has been deactivated/not found";
    }
