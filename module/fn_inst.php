...
{
    $inst_name = sanitize_text_field($_REQUEST["inst_name"]);
    $inst_cont_name = sanitize_text_field($_REQUEST["inst_cont_name"]);
    $inst_email = sanitize_text_field($_REQUEST["inst_email"]);
    $inst_website = sanitize_text_field($_REQUEST["inst_website"]);
    $inst_phone = sanitize_text_field($_REQUEST["inst_phone"]);
    $inst_fax = sanitize_text_field($_REQUEST["inst_fax"]);
    $inst_address = sanitize_text_field($_REQUEST["inst_address"]);
    $inst_zip = sanitize_text_field($_REQUEST["inst_zip"]);
    $inst_city = sanitize_text_field($_REQUEST["inst_city"]);
    $inst_state = sanitize_text_field($_REQUEST["inst_state"]);
    $inst_gmap = sanitize_text_field($_REQUEST["inst_gmap"]);
    $inst_attach_photo_id = sanitize_text_field($_REQUEST["photo_id"]);
    $inst_frnt_desc = sanitize_text_field($_REQUEST["inst_frnt_desc"]);
    $inst_meta_desc = sanitize_text_field($_REQUEST["inst_meta_desc"]);
    $inst_meta_keyword = sanitize_text_field($_REQUEST["inst_meta_keyword"]);
    $inst_meta_title = sanitize_text_field($_REQUEST["inst_meta_title"]);
    ...
    echo json_encode(array('success' => true, "msg" => "Institution details has been updated.", "header" => "OK", 'color' => 'success'));
    wp_die();
