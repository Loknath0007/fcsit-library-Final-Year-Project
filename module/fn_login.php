...
function FuncChangePassword()
{
    ...
    $password = sanitize_text_field($_REQUEST["new_pass"]);
    ...
    echo json_encode(array('success' => true, 'msg' => 'Password Changed Successfully', "header" => "Success", 'color' => 'success'));
    wp_die();
}
...
function getPassword()
{
    ...
    if (in_array('administrator', (array)$user->roles)) {
        ...
        echo json_encode(array('success' => true, "msg" => "Password has been reseted to the default password.", "header" => "OK", 'color' => 'success'));
        wp_die();
    }
    if ($password != "") {
        send_email_c($user_email, "Your Password For Library Management System is : " . $password);
    } else {
        echo json_encode(array('success' => true, "msg" => "Username / Email-Id doesn't match in our system.", "header" => "Info", 'color' => 'info'));
        wp_die();
    }
}
